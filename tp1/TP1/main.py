...
def exo2() -> dict:
    ...
    if not base_url:
        ...
        # to get measurement with measurement id : 38333397                #
        ####################################################################
        response = requests.get(f"{base_url}/{measurement_id}/results/").json()
...
def exo3() -> None:
    ...
    ####################################################################
    anchors: list = [1, 2, 3, 4, 5]
    base_url = "https://atlas.ripe.net/api/v2/anchors"
    response = requests.get(f"{base_url}/").json()
